...
//	{
	...
	//	_tcscpy(BMPName, "\\ColorCop5.bmp");
		hBitmap =(HBITMAP)LoadImage(AfxGetApp()->m_hInstance, strBMPFile, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
...
//		CreateBMPFile(curwindowhwnd, strBMPFile ,MagBmpInfo,hBitmap,::GetDC(NULL));
	}
...
void CHtmlcopDlg::OnOptionsOmitsymbol() 
{
	...
	if (m_Appflags & OmitPound)
		SetStatusBarText(IDS_SYMBOLOMIT);
	else
		SetStatusBarText(IDS_SYMBOLINCLUD);
