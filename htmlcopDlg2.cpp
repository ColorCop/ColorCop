/************************************************************************************
 *                                                                                  *
 * htmlcopDlg.cpp :: Color Cop - by Jay Prall - Jay@prall.net                    *
 *                                                                                  *
...
//	m_tooltip.AddTool(GetDlgItem(IDC_EYELOC),  IDS_EYEDROPPER_TOOLTIP);
	...
	if (m_Appflags & USECROSSHAIR) {
		...
		m_hEyeCursor = ::LoadCursor(NULL, IDC_CROSS);		// cross hair cursor
	...
	OnCopytoclip();	
	 ...
	 //GetDlgItem(IDC_HEXCOLOR)->SetFocus();	// set the focus to the Red edit control
	HWND testwnd;
	CWnd::GetDlgItem(IDC_RED,&testwnd);
	::PostMessage(testwnd,WM_KILLFOCUS,NULL,0);
	::PostMessage(testwnd,WM_SETFOCUS,NULL,0);
...
//	strBMPFile+=BMP_FILE;
	...
	// add ColorCop.dat
	strInitFile += BMP_FILE;


	szPath = strInitFile.GetBuffer(MAX_PATH);
...
*/
	...
	hBitmap =(HBITMAP)LoadImage(AfxGetApp()->m_hInstance, szPath, IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);

		// release the CString buffer / szPath
	strInitFile.ReleaseBuffer();
...
//	CWnd::GetDlgItem(IDC_SLIDE1, &temphandle);
//	::GetWindowRect(temphandle, &testrect);
//	CWnd::ScreenToClient(&testrect);
	...
	magplus.right = magplus.left + 9;
	magplus.bottom = magplus.top + 9;
	...
	magminus.right = magminus.left + 9;
	magminus.bottom = magminus.top + 9;
...
void CHtmlcopDlg::SetupTrayIcon()
{
	if (bMinimized && (pTrayIcon_ == 0) && (m_Appflags & MimimizetoTray)) {
		...
		pTrayIcon_->Create(0, nTrayNotificationMsg_, "Color Cop",
...
//	char hhh[80];
//	sprintf(hhh,"%d is allowed", abs(buttonrect.bottom - buttonrect.top));
//	sprintf(hhh,"Hue: %f, soff:%d", Hue, soff);
///	AfxMessageBox(hhh);
//	sprintf(hhh,"%d is allowed", abs(buttonrect.left - buttonrect.right));
//	AfxMessageBox(hhh);
...
//	Beep(100,50);
	...
	//	char tt[80];
	//	wsprintf(tt,"pt: %d, %d    left: %d",point.x,point.y,colorpalrect.left);
	//	SetWindowText(tt);
	...
	if (pWnd && pWnd->GetSafeHwnd() == m_EyeLoc.GetSafeHwnd()) {
       ...
       //  left mouse button down on the magnifier
		...
		wsprintf(maglevel," Magnifier increased to %dx", m_MagLevel);
		SetStatusBarText(maglevel);
		RecalcZoom();
		...
		if (m_MagLevel != 1)
			m_MagLevel--;
		wsprintf(maglevel," Magnifier decreased to %dx", m_MagLevel);
		SetStatusBarText(maglevel);
		RecalcZoom();
...
void CHtmlcopDlg::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	...
	} else if (pWnd && pWnd->GetSafeHwnd() == m_MagWindow.GetSafeHwnd()) 
	{
        if (hBitmap)
		{
			if (IDYES == AfxMessageBox(_T("Would you like to copy the bitmap to the clipboard?"), MB_YESNO))
			{
				if(::OpenClipboard(CCopHWND)) {
		...
		m_isMagPlusDown = TRUE;
		SetTimer(2,400,NULL);		// install the timer
        return;
...
void CHtmlcopDlg::OnMouseMove(UINT nFlags, CPoint point) 
{
		...
		if ((m_isEyedropping) || (m_isMagnifying)) {
			...
			if (::GetCapture() == NULL) { 
				...
				SetCapture();	// if we don't have capture, get it back
...
//				BlockMeasure(RelativePoint, RelativePointEnd);
			...
			TCHAR wtitle[64];
				...
				if (m_Appflags & Sampling3x3) {
					wsprintf(wtitle, " Eyedropping: %d, %d, 3x3 ", point.x, point.y);
				} else if (m_Appflags & Sampling5x5) {
					wsprintf(wtitle, " Eyedropping: %d, %d, 5x5 ", point.x, point.y);
				} else { // SampleRate == 1

					if (m_Appflags & DetectWebsafeColors) {	
						if (isWebsafeColor(m_Reddec, m_Greendec, m_Bluedec)) {
							wsprintf(wtitle, " Eyedropping: %d, %d (WebSafe)", point.x, point.y);
						} else {
							wsprintf(wtitle, " Eyedropping: %d, %d (not WebSafe)", point.x, point.y);
						}
					// end detect web
					} else {
						wsprintf(wtitle, " Eyedropping: %d, %d", point.x, point.y);
					}
				...
				if (bRelativePosition)  {
					...
					wsprintf(wtitle, " Relative Position: %d, %d", point.x - RelativePoint.x, point.y- RelativePoint.y);
					
					//wsprintf(wtitle, " Relative Position: %d, %d", point.x , point.y);
				}
				if (m_Appflags & MAGWHILEEYEDROP) {
		...
		wsprintf(wtitle, " Magnifying: %d, %d at %dx", point.x, point.y, m_MagLevel);
		SetStatusBarText(wtitle);
...
{	
    ...
    m_tooltip.RelayEvent(pMsg);	// tell the tooltip class about this too
	...
	if (pMsg->message == WM_KEYDOWN)
		if (pMsg->wParam == VK_ESCAPE) {
			...
			return TRUE;	// return control rather than letting it 
							// exit the app
		} else if ((pMsg->wParam == VK_CONTROL) && (m_isEyedropping)) {
			
			
			// Start Relative Positioning
	//	if (!bRelativePosition)
			if (GetCursorPos(&RelativePoint)) {
				RelativePointEnd = RelativePoint;
				TCHAR wtitle[64];
				wsprintf(wtitle, " Relative Position: 0, 0");
				SetStatusBarText(wtitle);
				CWinApp* pApp = AfxGetApp();		

				SetCursor(::LoadCursor(NULL,IDC_CROSS));		// Crosshair Cursor
				bRelativePosition = TRUE;						// Start Rela
...
void CHtmlcopDlg::OnDestroy() 
{
	...
	//szPath = strInitFile.GetBuffer(MAX_PATH);
	 szPath = (LPTSTR)(LPCTSTR)strInitFile;

/*	
// no more current directory
...
void CHtmlcopDlg::OnPopupColorConverttograyscale() 
{
	...
	if ((m_Reddec == m_Greendec)&&(m_Greendec == m_Bluedec)) {
		SetStatusBarText(_T("Color is already grayscale"));
	} else {

		SetStatusBarText(_T("Color Converted to grayscale"));
...
{
	m_StatBar.SetText("RGB Float Mode ON", 0, 0);
	if (m_Appflags ^ ModeHTML) {
		m_Appflags &= ~ModeHTML;			
...
{
	m_StatBar.SetText("RGB Int Mode ON", 0, 0);
	if (m_Appflags ^ ModeHTML)
	{
		m_Appflags &= ~ModeHTML;			
		m_Appflags &= ~ModeDelphi;
...
void CHtmlcopDlg::OnPopupOptionsUsecrosshaircursor() 
{
	...
	if (m_Appflags & USECROSSHAIR) {
		m_hEyeCursor = ::LoadCursor(NULL,IDC_CROSS);
		m_EyeLoc.SetIcon(pApp->LoadCursor(IDC_MYCROSS));
	} else {
